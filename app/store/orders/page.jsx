...
import { TruckIcon, PackageIcon, SearchIcon } from "lucide-react"
...
export default function StoreOrders() {
    ...
    const submitTracking = async (orderId) => {
        ...
        if (!trackingNumber?.trim()) {
            return toast.error('Masukkan nomor resi')
        }
        try {
            const token = await getToken()
            await axios.post('/api/store/orders/tracking', { orderId, trackingNumber }, { headers: { Authorization: `Bearer ${token}` } })
            setOrders(prev =>
                prev.map(order =>
                    order.id === orderId ? { ...order, trackingNumber, status: 'SHIPPED' } : order
                )
            )
            ...
            <div className="flex gap-1 overflow-x-auto pb-2 mb-5">
                {[
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 text-xs rounded-full whitespace-nowrap transition-all ${filterStatus === tab.key ? 'bg-slate-700 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                    >
                        {tab.label}
                ...
                <div className="overflow-x-auto max-w-5xl rounded-md shadow border border-gray-200">
                    <table className="w-full text-sm text-left text-gray-600">
                        <thead className="bg-gray-50 text-gray-700 text-xs uppercase tracking-wider">
                            <tr>
                                {["No", "Customer", "Total", "Metode Bayar", "Status Bayar", "Resi", "Status", "Tanggal"].map((heading, i) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-3">
                                        {order.status === 'CANCELLED' ? (
                                            <span className="bg-red-100 text-red-700 text-xs px-2 py-1 rounded-full">❌ Batal</span>
                                        ) : order.isPaid ? (
                                            <span className="bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full">✅ Lunas</span>
                                        ) : order.paymentMethod === 'COD' ? (
                                            <span className="bg-amber-100 text-amber-700 text-xs px-2 py-1 rounded-full">💵 Bayar di tempat</span>
                                        ) : (
                                    ...
                                    <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                                        {order.trackingNumber ? (
                                            ...
                                            <div className="flex items-center gap-1">
                                                ...
                                                />
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); submitTracking(order.id) }}
                                    ...
                                    <td className="px-4 py-3" onClick={(e) => { e.stopPropagation() }}>
                                        <select
                                            ...
                                            className="border-gray-300 rounded-md text-xs focus:ring focus:ring-blue-200 py-1"
                                        >
            ...
            {isModalOpen && selectedOrder && (
                <div onClick={closeModal} className="fixed inset-0 flex items-center justify-center bg-black/50 text-slate-700 text-sm backdrop-blur-xs z-50" >
                    <div onClick={e => e.stopPropagation()} className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6 relative max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="mb-4 p-3 bg-slate-50 rounded-lg text-sm">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                                ...
                                {selectedOrder.paymentMethod === 'MIDTRANS' ? (
                                    <strong className="text-slate-700">Transfer (Midtrans)</strong>
                                ) : (
                                    <strong className="text-slate-700">Bayar di Tempat (COD)</strong>
                                )}
                                <span className="text-slate-300">|</span>
                                {selectedOrder.status === 'CANCELLED' ? (
                                    <span className="bg-red-100 text-red-700 text-xs px-2.5 py-1 rounded-full">❌ Dibatalkan</span>
                                ) : selectedOrder.isPaid ? (
                                    <span className="bg-green-100 text-green-700 text-xs px-2.5 py-1 rounded-full">✅ Sudah Bayar</span>
                                ) : selectedOrder.paymentMethod === 'COD' ? (
                                    <span className="bg-amber-100 text-amber-700 text-xs px-2.5 py-1 rounded-full">💵 Bayar saat diterima</span>
                                ) : (
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className={`text-xs px-2.5 py-1 rounded-full ${STATUS_LABELS[selectedOrder.status]?.color || 'bg-slate-100 text-slate-600'}`}>
                        ...
                        {/* Customer Details */}
                        <div className="mb-4">
                            <h3 className="font-semibold mb-2">Detail Customer</h3>
                                ...
                                {selectedOrder.orderItems.map((item, i) => (
                                    <div key={i} className="flex items-center gap-4 border border-slate-100 shadow rounded p-2">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p>Harga: {currency}{item.price.toLocaleString('id-ID')}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="mb-4 p-3 bg-green-50 rounded-lg">
                            ...
                            <p className="text-xs text-slate-500">
                                Metode: {selectedOrder.paymentMethod} • Tanggal: {new Date(selectedOrder.createdAt).toLocaleString('id-ID')}
                            </p>
                        </div>
