...
export default function AdminAddProduct() {
    ...
    const handleImageUpload = async (key, file) => {
        ...
        if (key == "1" && file && !aiUsed) {
            ...
            reader.onloadend = async () => {
                ...
                try {
                    await toast.promise(
                        axios.post('/api/admin/ai', { base64Image: base64String, mimeType }, {
                            headers: { Authorization: `Bearer ${token}` }
                        }),
                        {
                            loading: "Menganalisis gambar dengan AI...",
                            success: (res) => {
                                const data = res.data
                                if (data.name && data.description) {
                                    ...
                                    setAiUsed(true)
                                    return "AI mengisi info produk 🎉"
                                }
                                return "AI tidak bisa menganalisis gambar"
                            },
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            if (!images[1] && !images[2] && !images[3] && !images[4]) {
                return toast.error("Upload minimal satu gambar")
            }
            setLoading(true)
    ...
    return (
        <form onSubmit={e => toast.promise(onSubmitHandler(e), { loading: "Menambahkan Produk..." })} className="text-slate-500 mb-28">
            ...
            <button disabled={loading} className="bg-green-600 text-white px-6 mt-7 py-2 hover:bg-green-700 rounded transition">
                Tambah Produk
            </button>
        </form>
    )
